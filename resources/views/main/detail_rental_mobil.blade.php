...
@section('content')
    ...
    <section class="flex gap-8 pt-12">
        <section class="px-20 w-[800px] flex flex-col gap-3">
            <img src="/storage/{{ $rental_mobil->image }}" alt=""
                class="w-[600px] rounded-xl h-[280px] object-cover overflow-hidden">
            <h1 class="text-2xl font-bold"> {{ $rental_mobil->nama }}</h1>
            <div class="w-[600px] flex justify-between">
                <h4 class="font-semibold">Rp {{ $rental_mobil->harga }}</h4>
                <h4 class="font-semibold">Rp {{ $rental_mobil->durasi }}</h4>
            </div>
            <div class="w-full">{!! $rental_mobil->konten !!}</div>
        </section>
        <section class="w-[450px] h-[500px] bg-pink-200">
            <form action="/pembayaran" method="post"
                ...
                <div class="w-full flex gap-2 flex-col">
                    <label for="tanggal" class="text-sm text-gray-600">Tanggal Rental</label>
