...
@section('content')
    <section class="h-screen flex flex-col px-20 py-12">
        <form action="/admin/paket-tour/edit/{{ $paket_tour->id }}" method="post" class="flex flex-col items-center gap-6 py-16 w-[80l00/.k=0px] bg-white rounded-xl shadow-lg px-16" enctype="multipart/form-data">
        @csrf
            <div class="text-purple-800 text-2xl font-bold">Edit Paket Tour</div>
            <div class="w-full flex gap-2 flex-col">
                <label for="nama" class="text-sm text-gray-600">Nama</label>
                <input type="text" id="nama" name="nama" class="w-full border-2 border-purple-600 py-2 px-4 rounded-md text-gray-600 outline-none" placeholder="nama paket" value="{{ $paket_tour->nama }}">
            </div>
            <div class="w-full flex gap-2 flex-col">
                <label for="deskripsi" class="text-sm text-gray-600">Deskripsi</label>
                <textarea type="text" id="deskripsi" name="deskripsi" class="w-full border-2 border-purple-600 py-2 px-4 rounded-md text-gray-600 outline-none" placeholder="deskripsi...">{{ $paket_tour->deskripsi }}</textarea>
            </div>
            <div class="w-full flex gap-2 flex-col">
                <label for="durasi" class="text-sm text-gray-600">Durasi</label>
                <div class="relative inline-block">
                    <select id="durasi" name="durasi" class="block appearance-none w-full py-2 px-4 pr-8 rounded-md leading-tight focus:outline-none focus:shadow-outline border-2 border-purple-600 text-gray-600" value="{{ $paket_tour->durasi }}">
            ...
            <div class="w-full flex gap-2 flex-col">
                ...
                <input type="text" id="harga" name="harga" class="w-full border-2 border-purple-600 py-2 px-4 rounded-md text-gray-600 outline-none" placeholder="Rp 1.200.000" value="{{ $paket_tour->harga }}">
            </div>
            <div class="w-full flex gap-2 flex-col">
                ...
                <div class="relative border-2 border-dashed border-purple-600 rounded-lg h-60">
                    <input type="file" id="image" name="image" class="absolute w-full h-full opacity-0" value="{{ $paket_tour->image }}">
                    <div id="image-preview" class="flex items-center justify-center h-full">
                        <img src="/storage/{{ $paket_tour->image }}" alt="Preview" class="h-full">
            ...
            <div class="w-full flex gap-2 flex-col">
                ...
                <input id="konten" type="hidden" name="konten" value={{ $paket_tour->konten }}>
